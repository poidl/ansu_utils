...
%     end
    ...
    neigh=circshift(ex,[0,1]);
    pts=isnan(ex) & ~isnan(neigh);
    ex(pts)=neigh(pts);
    neigh=circshift(ex,[0,-1]);
    pts=isnan(ex) & ~isnan(neigh);
    ex(pts)=neigh(pts);
    ...
    %ex(:,2:end)=0.5*(ex(:,1:end-1)+ex(:,2:end));
    %figure()
    %imagesc(ex);
...
%    imagesc(ey);
    ...
    neigh=circshift(ey,[1,0]);
    pts=isnan(ey) & ~isnan(neigh);
    ey(pts)=neigh(pts);
...
%      figure()
%     h=imagesc(weight);
%      set(h,'alphadata',~isnan(weight))
%      figure()
%      h=imagesc(sns);
%      set(h,'alphadata',~isnan(sns))
    
    
    % find corrected surface
    [sns, ctns, pns] = dz_from_phiprime(sns, ctns, pns, s, ct, p, phiprime,weight );
...
function [sns_out,ctns_out,pns_out] = dz_from_phiprime(sns, ctns, pns, s, ct, p, phiprime_e, weight );
...
vec=1.0+(1.0-1.0)*weight(:);
rho_surf=gsw_rho(sns(:),ctns(:),pns(:));
t2=rho_surf.*(1-vec.*phiprime_e(:)); % approximately t2.*exp(-r*phiprime_e)
