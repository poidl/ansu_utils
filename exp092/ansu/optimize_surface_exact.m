...
while it<=nit;
    ...
    % wetting
    % TODO: do wetting for the initial surface?
...
wet=~isnan(s(1,:,:)); % wet points at ocean surface
wets=~isnan(sns); % wet points on approx. neutr. surface (ans)
nn=wet(:) & circshift(wets(:),-1); % wet points adjacent to ans with north. neighbour on surface
...
%         pp=p(:,ii)
%     end
%    dbstop in depth_ntp at 233
    [sns(jj+yi),ctns(jj+yi),pns(jj+yi)] = depth_ntp(sns(ii),ctns(ii),pns(ii),s(:,jj+yi),ct(:,jj+yi),p(:,jj+yi)); 
end
...
for ii=transpose(find(nn));
    ...
    [sns(jj+1),ctns(jj+1),pns(jj+1)] = depth_ntp(sns(ii),ctns(ii),pns(ii),s(:,jj+1),ct(:,jj+1),p(:,jj+1)); 
end
