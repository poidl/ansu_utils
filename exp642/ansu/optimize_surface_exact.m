...
%     pns(pns<=cut_off_choice)=nan;
    ...
    if strcmp(error_measure,'drho_local')
        ...
        if use_b
            ...
            er_gradx=op_dXY(er_delx,'over','dx','i');
            er_grady=op_dXY(er_dely,'over','dy','j');
            erx=op_dXY(er_gradx,'times','sqrtdA','i'); % email 20140910
            ery=op_dXY(er_grady,'times','sqrtdA','j');
...
%             ery=er_dely;
            ...
            derr=0.8*derr;
            %derr=op_dXY(derr,'over','dA','none');
            %derr=op_dXY(derr,'over','sqrtdA','none');
            %derr=op_dXY(derr,'over','mean_dxy','none');
    ...
    elseif strcmp(error_measure,'slope_difference') 
        ...
        regions=find_regions(pns);
        %             [regions]=remove_points(er_delx,er_dely,pns);
        [derr,res]=solve_lsqr(regions, er_delx, er_dely);
        %derr=over_dA(derr);
...
if it>0
    ...
    % area weighted:
    one=ones(size(sns));
    dA=op_dXY(one,'times','dA','none');
    dA(isnan(sns))=nan;
    A=nansum(dA(:)); % total area
   
    gradx_times_sqrtdA=op_dXY(er_gradx,'times','sqrtdA','i'); % email 20140910
    grady_times_sqrtdA=op_dXY(er_grady,'times','sqrtdA','j');
