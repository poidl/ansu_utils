 function [sns,tns,pns,dsns,dtns,dpns] = neutral_surfaces_new(s,t,p,gamma,glevels)
 
%         [sns,tns,pns,dsns,dtns,dpns) = neutral_surfaces_new(s,t,p,gamma,glevels)   
%
...
for k = 1:n
    ...
    if length(inds)>0
      [sns(:,k),tns(:,k),pns(:,k),dsns(:,k),dtns(:,k),dpns(:,k)] = ...
                        neutral_surfaces0_new(s(inds,k),t(inds,k),p(inds,k),gamma(inds,k),glevels);
    end
end
