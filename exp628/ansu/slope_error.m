function [sx,sy]=slope_error(sns,ctns,pns,s,ct,p)
...
dz_east=(pe-pnse);
% dz at grid point
pw=circshift(pw,[0 0 -1]); % grid onto point to which tangent is laid
dz_orig=(pw-pns); 
...
dz_north=(pn-pnsn);
% dz at grid point
ps=circshift(ps,[0 -1 0]); % grid onto point to which tangent is laid
dz_orig=(ps-pns); 
