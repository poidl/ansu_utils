...
if it==0 % initialize
    ...
    ex_hist=nan(nit,yi,xi);
    ey_hist=nan(nit,yi,xi);
    
    
    vars = {'sns_hist','ctns_hist','pns_hist','eps_rms_hist','phiprime_e_hist','ex_hist','ey_hist'};
...
iteration_history.ex_hist(it+1,:,:) = ex;
iteration_history.ey_hist(it+1,:,:) = ey;
...
if strcmp(choice, 'epsilon')
    ...
    size(ex)
    %ex(1,end,:)=nan; % do it like we used to
    s1=ex(~isnan(ex));
    s2=ey(~isnan(ey));
    disp(['No. of nans in ex: ',num2str(sum(isnan(ex(:))))])
    disp(['No. of nans in ey: ',num2str(sum(isnan(ey(:))))])
    square=[s1(:) ; s2(:)].^2;
    slope_square(it+1,1) = nansum(square);
