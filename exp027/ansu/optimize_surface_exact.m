...
for it = 1:nit
    ...
    nn2= [zeros(1,xi);wet(1:end-1,:)] +0.; %south; 1 if has wet southern neighbour. Add zero to cast from logical to number.
    nn1= [wet(2:end,:);zeros(1,xi)] +0.; %north 
    switch wrap
        case 'none'
            nn4= horzcat(wet(:,2:end),zeros(yi,1)) +0.; %east
            nn3= horzcat(zeros(yi,1),wet(:,1:end-1)) +0.; %west
        case 'long'
            nn4= horzcat(wet(:,2:end),wet(:,1)) +0.; %east
            nn3= horzcat(wet(:,end),wet(:,1:end-1)) +0.; %west
    end
    ...
    nn3(~nn3)=nan;
    nn4(~nn4)=nan;
    neighbour(2,:)=(iwet+yi).*nn4(iwet); %east
    neighbour(5,:)=(iwet-1).*nn2(iwet); %south
    neighbour(4,:)=(iwet+1).*nn1(iwet); %north
    neighbour(3,:)=(iwet-yi).*nn3(iwet); %west
    % in case of longitudinal wrapping
    neighbour(neighbour>xi*yi)=neighbour(neighbour>xi*yi)-xi*yi;
    neighbour(neighbour<=0)=neighbour(neighbour<=0)+xi*yi;
