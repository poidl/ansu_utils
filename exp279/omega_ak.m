...
if initial_surface_at_constant_pressure;
    ...
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    pbar=nanmean(pns_out(:));
    rhohat=nanmean(gsw_rho(sns_out(:),ctns_out(:),pbar));
    glevels_Iak=rhohat;
    [zi,yi,xi]=size(sa);
    ...
    cnt=0;
      while 1
        ...
        F=gsw_rho(s_2,ct_2,pbar*ones(size(s_2)))-glevels_Iak;
        [s_2,ct_2,p_2,sns_out,ctns_out,pns_out, inds, fr]=root_core(F,inds,refine_ints,s_2,ct_2,p_2,sns_out,ctns_out,pns_out);
...
%dbstop in optimize_surface_exact at 260
