...
rhoalpha=rho_surf.*gsw_alpha(sns(:),ctns(:),pns(:));
rhobeta=rho_surf.*gsw_beta(sns(:),ctns(:),pns(:));
...
while 1
    ...
    if cnt==1 | cnt==2
        ...
        rho_surf_stacked=rho_surf(ii);
    end
     
     t2_stacked=t2_stacked(:,fr);

     rho_surf_stacked=rho_surf_stacked(:,fr);

    sns=sns(fr);
    ctns=ctns(fr);
    delta_s=bsxfun(@minus,s(:,:),sns(:)');
    delta_ct=bsxfun(@minus,ct(:,:),ctns(:)');
    
    rhobeta=rhobeta(fr);
    rhoalpha=rhoalpha(fr);
    
    % Linear equation of state
    t1=rho_surf_stacked+(bsxfun(@times,delta_s,rhobeta')-bsxfun(@times,delta_ct,rhoalpha'));
