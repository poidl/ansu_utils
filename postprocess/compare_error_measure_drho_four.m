...
fname='../exp254/data/iteration_history.mat';
varname= 'drho_rms_hist';
load(fname, varname);

vv4=drho_rms_hist; % variable to plot
...
p4=semilogy(xax,vv4,'k')
semilogy(xax,vv4,'k+','LineWidth',2)
power=[-11:-2];
set(gca, 'YTick', (10*ones(1,1:length(power))).^power )
grid on
legend([p1 p2 p3 p4],{'no modification','b at every iteration','b from first iteration','b from last iteration'});
xlabel('Iterations')
ylabel('$\sqrt{\overline{\Delta\rho^2}}\quad \rm [kg/m^{-3}]$','interpreter','latex','fontsize',13)
axis tight
print('-dpdf','-r200',['figures/compare_drho_rms_four'])