        ...
        while (isp<=nsp) && ( ip <=nit) && (iit<=size(vv,1))
            ...
            if colorscale{icolorscale}=='log'
               ...
               hold on
                ...
                xlabel(cb,'Red: log10($\tilde{r}-1>0$)  $\phantom{xxxxxx}$ Blue: log10( -1$\cdot(\tilde{r}-1<0$))','interpreter','latex','fontsize',18)
            elseif colorscale{icolorscale}=='lin'
                ...
                if isp==6 % 
                    for ii=1:nsp
                        ...
                        xlabel(cbh(ii),'$\tilde{r}$','interpreter','latex','fontsize',18)
                    end
                end
                hold on
