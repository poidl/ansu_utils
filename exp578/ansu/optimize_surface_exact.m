...
while it<=nit;
    ...
    if save_iterations;
        if it==0; % dummy values
            drhodx=nan; drhody=nan; drho=nan; res=nan; bns=nan; n2ns=nan; 
        end
        diagnose_and_write(it,sns,ctns,pns,drhodx,drhody,drho,res,bns,n2ns);
    end
...
%    dbstop in n2 at 6
    if 0
        ...
        % solve for delta rho
        [drho,res]=solve_lsqr(regions, drhodx_mod, drhody_mod);  
        drho=drho./b;
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
        ...
        bx=0.5*(circshift(bns,[0 -1])+bns);
        by=0.5*(circshift(bns,[-1 0])+bns);
        ...
        if 0
            ...
            [sns, ctns, pns] = depth_ntp_iter_drho_b(sns(:)', ctns(:)', pns(:)', s(:,:), ct(:,:), p(:,:), drho(:)', b(:,:), p(:,:) );
            drho=drho./bns; % this is just to compare drho with experiments in which it is not used in vertical integration.
        end
        
        
    else
        %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
        % b is only needed here to set the surface to nan where b is
        % undefined, for fair comparison with experiments with b.
        load('data/b.mat','b');
        %b=b(1:4:end,:,:);
        p0=pns(:)';
        ...
        %[drho,res]=solve_lsqr_nostation(regions, drhodx, drhody);
        % find corrected surface
        [sns, ctns, pns] = depth_ntp_iter_drho(sns(:)', ctns(:)', pns(:)', s(:,:), ct(:,:), p(:,:), drho(:)');
