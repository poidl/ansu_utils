...
for ii=transpose(find(en));
    ...
    [sns(ii),ctns(ii),pns(ii)] = depth_ntp(sns(jj+yi),ctns(jj+yi),pns(jj+yi),s(:,ii),ct(:,ii),p(:,ii)); 
end
...
for ii=transpose(find(nn));
    ...
    [sns(ii),ctns(ii),pns(ii)] = depth_ntp(sns(jj+1),ctns(jj+1),pns(jj+1),s(:,ii),ct(:,ii),p(:,ii)); 
end
