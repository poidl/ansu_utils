...
elseif strcmp(keyword,'bp')
        ...
        [gradx_p,grady_p] = grad_surf(pns,e1t,e2t,'bp');
        ...
        ss=gsw_sound_speed(sns(:,:),ctns(:,:),pns(:,:));
        rho=gsw_rho(sns(:,:),ctns(:,:),pns(:,:));
        drdp=1./ss.^2; 
        kappa=(1./rho).*drdp;
        kappa=reshape(kappa,[zz,yy,xx]);
        ...
        kappa_x=0.5*( kappa + circshift(kappa,[0 0 -1]) );
        kappa_y = 0.5*( kappa + circshift(kappa,[0 -1 0]) );
        ...
        ex = ((beta_x .* gradx_s) - (alpha_x .* gradx_ct) + (kappa_x.*gradx_p));
        ey = ((beta_y .* grady_s) - (alpha_y .* grady_ct) + (kappa_y.*grady_p));
