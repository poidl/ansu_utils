...
if it==0 % initialize
    ...
    drhodx_hist = nan(nit,yi,xi);
    drhody_hist = nan(nit,yi,xi);
    drho_hist = nan(nit,yi,xi);
    
    b_hist = nan(nit,yi,xi);
    n2ns_hist = nan(nit,yi,xi);
    
    vars = {'sns_hist','ctns_hist','pns_hist','drhoxy_rms_hist','drho_rms_hist',...
       'drhodx_hist','drhody_hist','drho_hist','b_hist','n2ns_hist'};
...
if it>0
    iteration_history.drhodx_hist(it,:,:) = permute(drhodx,[3,1,2]);
    iteration_history.drhody_hist(it,:,:) = permute(drhody,[3,1,2]);
