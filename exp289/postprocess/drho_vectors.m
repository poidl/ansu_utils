...
vx=0.5*(vx+circshift(vx,[0 0 -1]));
vy=0.5*(vy+circshift(vy,[0 -1 0]));
vx=circshift(vx,[0 0 1]);
vy=circshift(vy,[0 1 0]);

[nit,yi,xi]=size(vx);
arrows=true(yi,xi); 
arrows([1 yi],:)=false; 
% non periodic?: arrows(:,[1 xi])=false;

ll=squeeze(lats(1,:,:));
arrow_lats=ll(arrows(:));
ll=squeeze(longs(1,:,:));
arrow_longs=ll(arrows(:));
...
%                 xlabel(cb,'Red: log10($\Phi''>0$)  $\phantom{xxxxxx}$ Blue: log10( -1$\cdot(\Phi''<0$))','interpreter','latex','fontsize',18)
            ...
            elseif colorscale{icolorscale}=='lin'
                ...
                quiver(arrow_longs,arrow_lats,vx(iit,:)',vy(iit,:)',2)
        ...
        print('-dpng','-r200',['figures/drho_vectors_',num2str(ifig,'%02i')])
    end
end
