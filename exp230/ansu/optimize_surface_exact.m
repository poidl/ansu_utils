...
while it<=nit;
    ...
    if save_iterations;
        if it==0; % dummy values
            drho=nan; drhody_p=nan;
        end
        diagnose_and_write(it,sns,ctns,pns,drho,drhody_p);
    end
...
%     end
    ...
    [drhodx,drhody,drhody_p]=delta_tilde_rho(sns,ctns,pns);
...
function diagnose_and_write(it,sns,ctns,pns,drho,drhody_p)
...
if it==0 % initialize
    ...
    drhody_p_hist = nan(nit,yi,xi);
    
    vars = {'sns_hist','ctns_hist','pns_hist','drho_rms_hist','drho_hist','drhody_p_hist'};
...
if it>0
    ...
    iteration_history.drhody_p_hist(it,:,:) = permute(drhody_p,[3,1,2]);
