...
g = grav(squeeze(lats(1,:,:)));

[n2,p_mid] = bfrq_ct(sa,ct,p,g);
...
%todo: gpoly16ct takes sp, not sa
rho = gpoly16ct(sa,ct);
poly = rho;
...
%    display(['Iak',int2str(Iak)])
 %   disp(glevels(Iak))
        ...
        tic
        [sns_i(Iak,:,:),ctns_i(Iak,:,:),pns_i(Iak,:,:)] = optimize_surface_exact(sa,ct,p,g,n2,sns(Iak,:,:),ctns(Iak,:,:),pns(Iak,:,:),e1t,e2t,settings);
